        ...
        switch (attribute_id)
        {
            case stun_attribute_id_t::mapped_address:
            {
                if (payload_size >= 8)
                {
                    ...
                    else if (ip_type == 2 && payload_size >= 20) // ip 6
                    {
                        ip_address_t::ip6_address_t address(stun_message.payload(4));
                        attribute = stun_attribute_mapped_address_t::create({ ip_address_t::ip6_address_t(address), port });
                    }
                }
            }
            break;
            ...
            case stun_attribute_id_t::xor_mapped_address:
                if (payload_size >= 8)
                {
                    ...
                    else if (ip_type == 2 && payload_size >= 20) // ip 6
                    {
                        ...
                        xor_data(address.address.data()
                                 , xor_mapped
                                 , address.address.size());

                        attribute = stun_attribute_xor_mapped_address_t::create({ address, port });
    ...
    for (const auto& a : attributes)
    {
        ...
        switch(a->attribute_id)
        {
            case stun_attribute_id_t::mapped_address:
            {
                ...
                {
                    switch(mapped_address_attribute.endpoint.address.version)
                    {
                        ...
                        case ip_version_t::ip6:
                        {
                            ...
                            .append(utils::endian::big::convert<std::uint16_t>(payload_size))
                            .padding(1)
                            .append<std::uint8_t>(2)
                            .append<std::uint16_t>(utils::endian::big::convert<std::uint16_t>(mapped_address_attribute.endpoint.port))
                            .append(mapped_address_attribute.endpoint.address.ip6_address.address.data()
                                    , mapped_address_attribute.endpoint.address.ip6_address.address.size());
                        }
                        break;
                        default:;
                    }
            ...
            case stun_attribute_id_t::xor_mapped_address:
            {
                ...
                if (xor_mapped_address_attribute.endpoint.is_valid())
                {
                    ...
                    switch(xor_mapped_address_attribute.endpoint.address.version)
                    {
                        case ip_version_t::ip4:
                        {
                            auto address = xor_mapped_address_attribute.endpoint.address.ip4_address.address;
                            reinterpret_cast<std::uint8_t*>(&address)[3] ^= xor_mapped[0];
                            reinterpret_cast<std::uint8_t*>(&address)[2] ^= xor_mapped[1];
                            reinterpret_cast<std::uint8_t*>(&address)[1] ^= xor_mapped[2];
                            reinterpret_cast<std::uint8_t*>(&address)[0] ^= xor_mapped[3];
                        ...
                        case ip_version_t::ip6:
                        {
                            auto address = xor_mapped_address_attribute.endpoint.address.ip6_address;
                            ...
                            .append<std::uint8_t>(2)
                            .append<std::uint16_t>(utils::endian::big::convert<std::uint16_t>(port))
                            .append(xor_mapped_address_attribute.endpoint.address.ip6_address.address.data()
                                    , xor_mapped_address_attribute.endpoint.address.ip6_address.address.size());
                        }
                        break;
                        default:;
                    }
                           ...
                           , stun_message_t &message)
{
    ...
    if (stun_message.is_valid(parse_data.size()))
    {
        ...
        if (detail::parse_attributes(stun_message
                             , message.attributes))
        {
